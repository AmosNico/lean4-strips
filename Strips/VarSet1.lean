...
@[unbox, ext]
structure VarSetIterator (n : ℕ) where
  /-- The set of variables that is iterated over. -/
  varSet : VarSet n
  /-- All variable less then `pos` are ignored. -/
  pos : ℕ

namespace Iterator

@[always_inline, inline]
instance instVarSetIterator {n m} [Pure m] : Std.Iterator (VarSetIterator n) m (Fin n) where
  IsPlausibleStep it
    | .yield it' out =>
      it.internalState.varSet = it'.internalState.varSet ∧
      it'.internalState.pos = it.internalState.pos + 1 ∧
      out = it'.internalState.pos ∧ ∃ h : out < n, ⟨out, h⟩ ∈ it.internalState.varSet
    | .skip it' =>
      it.internalState.varSet = it'.internalState.varSet ∧
      it'.internalState.pos = it.internalState.pos + 1 ∧
      ∃ h : it'.internalState.pos < n, ⟨it'.internalState.pos, h⟩ ∉ it.internalState.varSet
    | .done => it.internalState.pos ≥ n
